...
class ArticleMySQLStorage
{
    ...
    {
        $sth = $this->dbh->prepare('SELECT * FROM articles WHERE id_article = :id');
        $sth->execute([':id' => $id]);
    ...
    public function update(int $id, string $marque, string $reference, string $designiation, float $prixUnitaire, int $qteStock, ?string $couleur = null, ?int $grammage = null)
    {
        $sth = $this->dbh->prepare('
            UPDATE articles
            SET reference = :reference, marque = :marque, designation = :designation, prix_unitaire = :prix_unitaire, qte_stock = :qte_stock, couleur = :couleur
    ...
    public function findAll(string $sortBy, string $direction = 'ASC'): array
    {
        ...
        $sth = $this->dbh->prepare("SELECT * FROM articles ORDER BY $sortBy $direction");
        $sth->execute();
    ...
    {
        $sth = $this->dbh->prepare('SELECT * FROM articles WHERE type = :type');
